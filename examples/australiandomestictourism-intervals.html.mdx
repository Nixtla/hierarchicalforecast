...
$\mathbf{y}_{[a,b]\tau}$ 2. `S_df`: the aggregation constraints
dataframe with $S_{[a,b]}$ 3. `tags`: a list with the ‘unique_ids’
conforming to each aggregation level.
...
in `Y_df` using the `AutoARIMA` model. Observe that `Y_hat_df` contains
the forecasts but they are not coherent. To reconcile the prediction
intervals we need to calculate the incoherent intervals using the
`level` argument of `StatsForecast`.
