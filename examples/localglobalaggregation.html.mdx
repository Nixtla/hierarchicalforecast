...
only lookking at the timestamps of each series, disregarding the
...
the second timeseries (as it is shorter), we have only have one record
for the aggregation.
...
-   The `global` behavior can be useful when dealing with timeseries
    ...
    not always have sales for all timeesteps, but one is interested in
