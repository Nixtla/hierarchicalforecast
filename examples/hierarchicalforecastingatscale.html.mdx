...
For the best performance, pass the returned `SMatrix` directly to
`reconcile()`. The library knows that matrices created by `aggregate()`
are valid, so repeated reconciliation calls avoid unnecessary checks.
...
either way, so `sparse_s=True` is a safe explicit choice.
