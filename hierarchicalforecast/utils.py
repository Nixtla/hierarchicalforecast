...
# %% auto 0
__all__ = ['aggregate', 'aggregate_temporal', 'HierarchicalPlot']
...
def aggregate_temporal(
    ...
    id_time_col = id_col + f"_{time_col}"
    Y_df_te, S_df_te, tags_te = aggregate(
        df=df,
        spec=spec,
        exog_vars=exog_vars,
        sparse_s=sparse_s,
    ...
    Y_nw_te = nw.from_native(Y_df_te)

    # Add ds column to Y_df_te
    # This is a bit dirty but Narwhals does not support string splitting
    timestamps = pd.to_datetime(
        Y_nw_te[id_time_col].to_pandas().str.split("/").str[-1], format="mixed"
    ).values
    Y_nw_te = Y_nw_te.with_columns(**{time_col: timestamps})
