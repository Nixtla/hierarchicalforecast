...
# %% auto 0
__all__ = ['concat_str', 'aggregate', 'HierarchicalPlot']
          ...
          # Update agg_dict to handle multiple aggregations for each exog_vars key
            for key, agg_func in exog_vars.items():
                ...
                for func in agg_func:
                    agg_dict[f"{key}"] = func  # Update the agg_dict with the new naming structure
                    exog_var_names.append(f"{key}")


    # Perform the aggregation
    for levels in spec:
        agg = ufp.group_by_agg(df, by=levels + ["ds"], aggs=agg_dict)
