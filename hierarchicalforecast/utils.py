...
def concat_str(
    ...
    """
    Concatenate multiple columns horizontally into a single string column.
    """
    if isinstance(df, pd.DataFrame):
        out = df[cols].astype(str).agg(separator.join, axis=1)
    else:
        out = df.with_columns(
            pl.concat_str( 
                [pl.col(col) for col in cols],
                separator=separator,
            ).alias(concat_col_name))[concat_col_name]
    return out

def group_by_agg_named(df: DataFrame, by, aggs, maintain_order=False) -> DataFrame:
    """	
    Group by and aggregate columns with named aggregations.
    """
    if isinstance(df, pd.DataFrame):
        out = ufp.group_by(df, by, maintain_order).agg(**aggs).reset_index()
    else:
        out = ufp.group_by(df, by, maintain_order).agg(
...
def aggregate(
    ...
    Returns
    -------
    Y_df : pandas or polars DataFrame
        Hierarchically structured series.
    S_df : pandas or polars DataFrame
        Summing dataframe.
    tags : dict
        Aggregation indices.
    """
    ...
    for col in df.columns:
        assert not ufp.is_none(df[col]).any(), f"Column {col} contains null values. Make sure no column in the DataFrame contains null values."            
            
    if sparse_s and isinstance(df, pl.DataFrame):
        raise ValueError("Sparse output is not supported for Polars DataFrames.")
    ...
    for levels in spec:
        agg = group_by_agg_named(df, by=levels + ["ds"], aggs=agg_dict)
        unique_ids = concat_str(agg, levels, "unique_id")
        agg = ufp.assign_columns(agg, names="unique_id", values=unique_ids)
        agg = ufp.drop_columns(agg, levels)
        agg = ufp.sort(agg, by=["unique_id", "ds"])        
        tags['/'.join(levels)] = np.array(list(sorted(set(unique_ids))))
        aggs.append(agg)

    Y_df = ufp.vertical_concat(aggs)
    Y_df = Y_df[["unique_id", "ds", "y"] + exog_var_names]	
    ...
    **Parameters:**<br>
    ...
    def __init__(self,
        ...
        if isinstance(S, pl.DataFrame):
            S = S.to_pandas().set_index("unique_id")
        else:
            S = S.set_index("unique_id")
        self.S = S
        self.tags = tags
    ...
    def plot_series(self,
        ...
        **Returns:**<br>
        ...
        """
        # Temporary hack to convert polars to pandas
        if isinstance(Y_df, pl.DataFrame): 
            Y_df = Y_df.to_pandas().set_index('unique_id')
        else:
        ...
        df_plot = Y_df.query("unique_id == @series").set_index('ds')
